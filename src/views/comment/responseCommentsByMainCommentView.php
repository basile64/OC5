<div class="response-comments-container">
    <?php foreach ($mainComment->getResponseComments() as $responseComment) : ?>
        <div class="avatar-plus-header-response">
            <a class="avatar" href="<?= htmlspecialchars(BASE_URL) ?>user/<?= htmlspecialchars($responseComment->getUser()->getId()) ?>"><img src="<?= htmlspecialchars(BASE_URL) ?>public/avatar/<?= htmlspecialchars($responseComment->getUser()->getAvatar()) ?>" class="avatar"></a>
